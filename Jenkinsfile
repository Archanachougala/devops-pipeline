    ...
    post {
        ...
        success {
            ...
            sh '''
                ...
                APP_PUBLIC_IP=$(curl -s http://169.254.169.254/latest/meta-data/public-ipv4 2>/dev/null || echo "APP_SERVER")
                    ...
                    -d "{
                        ...
                        \\"blocks\\": [
...

                            {
                                ...
                                \\"text\\": {\\"type\\": \\"mrkdwn\\", \\"text\\": \\"*Status:*\\\\nDeployment successful. App is live at http://32.199.151.13:5000\\"} 
            ...
            withCredentials([string(credentialsId: 'SLACK_WEBHOOK_URL', variable: 'SLACK_URL')]) {
                ...
                sh '''
                    ...
                    if [ "$PREVIOUS" != "none" ] && [ -n "$PREVIOUS" ]; then
                        ...
                        HEADER="⚠️ Deployment ${VERSION} — Rolled Back"
                        ...
                        HEADER="❌ Deployment ${VERSION} — Failed"
