...
import os, socket, datetime, subprocess, psutil, json, boto3
...
def get_docker_status():
    try:
        ...
        if out and "devops-app" in out:
            return "green", "green", "RUNNING"
        # try checking via running container list
...
def get_container_info():
    try:
        ...
        policy = run_cmd(["docker", "inspect", cid, "--format", "{{.HostConfig.RestartPolicy.Name}}"])
        return {
        ...
        return {"name": "devops-app", "image": f"devops-app:{VERSION}",
                "status": "running", "restart": "always"}
...
def home():
    ...
    return render_template("index.html",
        version=VERSION, environment=ENV_NAME,
        hostname=socket.gethostname(), host_ip=host_ip,
        current_time=datetime.datetime.utcnow().strftime("%Y-%m-%d %H:%M:%S UTC"),
        uptime=get_uptime(), total_deploys=total, successful_deploys=success,
        docker_status=docker_lbl,  docker_dot=docker_dot,  docker_badge_class=docker_badge,
        ansible_status="SUCCESS",  ansible_dot="green",    ansible_badge_class="green",
        jenkins_status=jenkins_lbl, jenkins_dot=jenkins_dot, jenkins_badge_class=jenkins_badge,
        health_status="HEALTHY",   health_dot="green",     health_badge_class="green",
        health_stage_class="done", health_stage_icon="✅",
        cpu_percent=metrics["cpu_percent"],   mem_percent=metrics["mem_percent"],
        disk_percent=metrics["disk_percent"], total_ram=metrics["total_ram"],
        used_ram=metrics["used_ram"],         free_ram=metrics["free_ram"],
        container_name=cinfo["name"],         container_image=cinfo["image"],
        container_status=cinfo["status"],     container_restart=cinfo["restart"],
...
def info():
    metrics = get_system_metrics()
    return jsonify({
        "app": "DevOps Rollback Pipeline", "version": VERSION,
        "environment": ENV_NAME, "uptime": get_uptime(),
...
def container_details():
    try:
        ...
        return jsonify({
            ...
            "Image":   image if image else f"devops-app:{VERSION}",
            "Status":  status if status else "running",
            "Started": started[:19].replace("T"," ") if started else "unknown",
            "Restart": policy if policy else "always",
            "Port":    "0.0.0.0:5000",
            "Version": VERSION
        })
